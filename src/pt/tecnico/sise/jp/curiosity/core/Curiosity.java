...
public class Curiosity {
	...
	public static void configLandingSite(int landingLine, int landingColumn, int numLines, int numColumns) {
		...
		// You are supposed to use/save initial information to be used in the controlRobot method.
		robot = new RobotState(landingColumn,landingLine,RobotController.getEnergy());
		map = new MarsObjects[numLines][numColumns];
		Arrays.fill(map, unknown);
		map[landingLine-1][landingColumn-1] = space;
	}
	...
	public static int[] checkAround(String side){
		...
		case NORTH: return new int[]{robot.getRobotPositionX()+1,robot.getRobotPositionY()};
		case WEST: return new int[]{robot.getRobotPositionX(),robot.getRobotPositionY()-1};
		case SOUTH: return new int[]{robot.getRobotPositionX()-1,robot.getRobotPositionY()};
		case EAST: return new int[]{robot.getRobotPositionX()-1,robot.getRobotPositionY()};
		default :  return new int[]{robot.getRobotPositionX()+1,robot.getRobotPositionY()};
		}
		case:()
		}
